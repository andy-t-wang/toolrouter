...
- Supabase Auth emails should not expose the raw Supabase verify URL. The confirmation template uses `{{ .SiteURL }}/auth/confirm?token_hash={{ .TokenHash }}&type=signup&redirect_to={{ .RedirectTo }}`, and the Next route at `/auth/confirm` calls `verifyOtp` before redirecting to the dashboard with the session fragment.
