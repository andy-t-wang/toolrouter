...
const endpointStatusFilter = readFileSync(new URL("../../../apps/web/app/endpoint-status-filter.tsx", import.meta.url), "utf8");
...
describe("web dashboard static wiring", () => {
  it("keeps the public landing page at the root route", () => {
    ...
    assert.doesNotMatch(landingPage, /statusReason/);
    assert.doesNotMatch(landingPage, /LayerChips/);
    assert.match(endpointStatusFilter, /agentKitBenefit\(provider\)/);
    assert.match(endpointStatusFilter, /Free trial/);
    assert.match(endpointStatusFilter, /Access/);
    assert.match(endpointStatusFilter, /<div>Benefit<\/div>/);
    assert.match(endpointStatusFilter, /Last check/);
    assert.match(landingPage, /Checked at least once/);
    assert.match(landingPage, /categoryTabsFromEndpoints/);
    assert.match(endpointStatusFilter, /Endpoint categories/);
    assert.match(endpointStatusFilter, /endpoint-tabs/);
    assert.match(endpointStatusFilter, /window\.history\.pushState/);
    ...
    assert.doesNotMatch(landingPage, /categoryTabHref\(category\.id\)/);
    assert.match(endpointStatusFilter, /visibleEndpoints\.map/);
    assert.match(landingPage, /isRecommendedEndpoint\(provider\)/);
    assert.match(endpointStatusFilter, /Recommended default endpoint/);
    ...
    assert.doesNotMatch(endpointStatusFilter, /<div>Latency<\/div>/);
    assert.match(endpointStatusFilter, /displayEndpointId\(provider\)/);
